...
class TD3Config:
    def __init__(self) -> None:
        self.algo_name = 'TD3 with LSTM AGA'  # 算法名称
        ...
        self.gamma = 0.99  # TODO (改了这里)gamma factor
        ...
        self.n_actions = 3
        self.max_action = 1.0
        self.memory_window_length = 20  # 记忆窗口的长度
        self.seed = 1
...
def eval(cfg, client, agent):
    ...
    for i_step in range(cfg.max_step):
        ...
        next_state, reward, actor_reward, done = env.step(action)
...
def train(cfg, client, agent):
    ...
    for i_ep in range(int(cfg.train_eps)):
        ...
        for i_step in range(cfg.max_step):
            ...
            agent.memory.push(history, state, action, next_state, reward, actor_reward, done)
            ...
            print('\rEpisode: {}\tStep: {}\tReward: {:.2f}\tDistance: {:.2f}'.format(i_ep + 1, i_step + 1, ep_reward,
                                                                                     state[3] * env.max_distance),
                  end="")
            final_distance = state[3] * env.max_distance
            if done:
                if result == 1:  # success
...
# 对20221130-181802可取前2500
if __name__ == "__main__":
    cfg = TD3Config()
    set_seed(cfg.seed)
    make_dir(cfg.result_path, cfg.model_path, cfg.ab_model_path)
