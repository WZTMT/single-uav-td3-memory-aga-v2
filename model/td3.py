...
class TD3(object):
    def __init__(self, cfg):
        ...
        self.actor = Actor(cfg.n_states, cfg.n_actions, self.max_action, self.device, self.n_move, self.n_sensor).to(self.device)
    ...
    def update(self):
        ...
        history, state, action, reward, actor_reward, next_state, done = self.memory.sample(self.batch_size)
        ...
        actor_reward = torch.FloatTensor(actor_reward).unsqueeze(-1).to(self.device)
        ...
        if self.total_iteration % self.policy_freq == 0:
            ...
            # Compute actor loss
            actor_loss = -self.critic.q1(his, state, self.actor(his, state)).mean() + self.alpha * actor_reward.mean()
