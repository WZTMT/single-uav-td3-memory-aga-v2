...
class ReplayBuffer(object):
    ...
    def push(self, history, state, action, next_state, reward, actor_reward, done):
    # def push(self, history, state, action, next_state, reward, done):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = (history, state, action, reward, actor_reward, next_state, done)  # 替换存入的None
    ...
    def sample(self, batch_size):
        ...
        history, state, action, reward, actor_reward, next_state, done = zip(*batch)
        # history, state, action, reward, next_state, done = zip(*batch)

        return history, state, action, reward, actor_reward, next_state, done
