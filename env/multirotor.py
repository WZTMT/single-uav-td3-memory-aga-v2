...
class Multirotor:
    ...
    def get_distance_sensors_data(self):
        ...
        data = []
        prefix = "Distance"
        data.append(20 - self.client.getDistanceSensorData(distance_sensor_name=prefix + 'S').distance)
        for i in yaw_axis:
            dsn = prefix + 'Y' + i
            data.append(20 - self.client.getDistanceSensorData(distance_sensor_name=dsn).distance)
        for i in pitch_axis:
            dsn = prefix + 'P' + i
            data.append(20 - self.client.getDistanceSensorData(distance_sensor_name=dsn).distance)
    ...
    def step(self, action):
        ...
        # speed_reward = self.speed_reward(ax, ay)
        reward = arrive_reward + yaw_reward + max_sensor_reward + num_sensor_reward + collision_reward + step_reward + distance_reward + cross_border_reward
        ...
        return next_state, reward, actor_reward, done, result
        # return next_state, reward, done, result

    '''
    输出是否完成和完成标识
    0表示未完成，1表示成功， 2表示碰撞（触及边界包括在内）
    '''
    def if_done(self):
        # 与目标点距离小于25米
    ...
    '''
    加在actor_loss上的约束，用于速度控制
    '''
    def actor_reward(self):
        sx = 0.0167 * pow(abs(self.vx), 2) - 0.1336 * abs(self.vx) + 0.2505
        sy = 0.0167 * pow(abs(self.vy), 2) - 0.1336 * abs(self.vy) + 0.2505

        return sx + sy

    def speed_reward(self, ax, ay):
        srx = 0
        sry = 0
        sr = 0  # 速度过小惩罚
        if (self.vx > 5 and ax > 0) or (self.vx < -5 and ax < 0):
            srx = -0.2
        if (self.vy > 5 and ay > 0) or (self.vy < -5 and ay < 0):
            sry = -0.2
        if -3 < self.vx < 3 or -3 < self.vy < 3:
            sr = -0.1
    ...
    def max_sensor_reward(self):
        data = self.get_distance_sensors_data()
        d_max = max(data)
        if d_max > self.d_safe:
            return math.exp((d_max - self.d_safe) / -29) - 1.3
        else:
            return 0
